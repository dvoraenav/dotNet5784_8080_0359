...
internal class EngineerImplementation : IEngineer
{
    ...
    public void Update(BO.Engineer item)
    {
        ...
        try
        {
            if (item.Task is not null)         //there is task that assigned to the engineer
            {
    ...
    public IEnumerable<BO.Engineer> GetEngineerList(Func<BO.Engineer, bool> filter = null)
    {
        ...
        return (from engineer in _dal.Engineer.ReadAll()//going into the engineers list and build a new entitiy for everyone
                select new BO.Engineer()
                {
                    ...
                    Task = getTaskInEngineer(engineer.Id)//find the current task of the engineer
                }).Where(engineer => filter is null ? true : filter(engineer));//we can ask for specific engineers
    }
